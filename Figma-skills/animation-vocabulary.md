...
Source skill: `animation-vocabulary` (installed from github.com/emilkowalski/skills). A reverse-lookup glossary: turns a vague description of a web animation/motion effect into its exact term. Use before writing a Figma Make prompt so you can name the effect precisely instead of describing it loosely.
...
- **Continuity transition** — A change that keeps the user oriented by visually connecting before and after (making the same rectangle bigger/smaller).
...
1. Read for intent, not keywords — users describe what they *see/feel* ("springy," "slides off," "draws itself in"), not the technical name.
2. Quote the glossary description verbatim rather than paraphrasing.
3. Disambiguate close terms (Clip-path vs Mask, Pop in vs Bounce, Shared element transition vs Layout animation).
4. When nothing matches exactly, name the closest term and say it's an approximation.
5. Stay within the glossary — if a term genuinely isn't here, say so rather than inventing one.

**Example:** "What's it called when a popover seems to grow out of the button you clicked instead of from its middle?" → **Origin-aware animation** — an element animates out of its trigger, like a popover growing from the button that opened it instead of from its own center (the CSS default).
